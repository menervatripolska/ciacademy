...
// ============================================================================
// PORTFOLIO COMPOSITION — что мы держим и почему.
// Заменяет CryptoMap: каждая строка — с объяснением "как читать".
// Монета недели подтягивается из coinOfTheWeek отдельно, эти 6 — ядро.
// В будущем эти цифры будет тянуть Worker из Airtable (base appbRhvPzqLHYCtDH).
// ============================================================================

export type PortfolioAssetKind = "crypto" | "metal" | "commodity" | "index" | "coin-of-week";

export interface PortfolioAsset {
  id: string;                // уникальный ключ
  kind: PortfolioAssetKind;
  symbol: string;            // тикер / короткое имя
  name: string;              // полное имя
  sector: string;            // что это по сути
  // Текущая цена в долларах (источник — в thesis/howToRead)
  priceUsd: number;
  change24h: number;         // %
  change7d: number;          // %
  // Доля в целевой аллокации
  allocationPct: number;
  // Зачем эта позиция вообще в портфеле
  thesis: string;
  // «Как читать» — 2-3 предложения на expand
  howToRead: string;
  // Что делаем сейчас (HOLD/ACCUMULATE/REDUCE)
  action: string;
}

export const portfolioComposition: PortfolioAsset[] = [
  {
    id: "btc",
    kind: "crypto",
    symbol: "BTC",
    name: "Bitcoin",
    sector: "Резерв · цифровое золото",
    priceUsd: 68420,
    change24h: 1.2,
    change7d: 4.8,
    allocationPct: 40,
    thesis:
      "Ядро портфеля. Халвинг отработал, структурный спрос через ETF не остановился, майнинг на исторически высокой себестоимости. BTC — единственный актив, на котором строятся все остальные решения в крипте.",
    howToRead:
      "Смотрим на BTC первой. Его доминация и фаза цикла (Pi Cycle, Hash Ribbons) задают режим для всего портфеля. Если BTC в бычьей фазе — остальные позиции работают на умножение; если в коррекции — режем риск и ждём.",
    action: "HOLD · Стандарт x1 — без усиления, без ослабления.",
  },
  {
    id: "eth",
    kind: "crypto",
    symbol: "ETH",
    name: "Ethereum",
    sector: "Смарт-контракт платформа #1",
    priceUsd: 3240,
    change24h: 0.7,
    change7d: 3.1,
    allocationPct: 18,
    thesis:
      "Эфир — расчётный слой DeFi, RWA, стейблов. Активация Pectra и рост L2 TVL возвращают fee в ETH. Соотношение ETH/BTC разворачивается от 3-летнего дна.",
    howToRead:
      "ETH/BTC — ключ. Если этот курс растёт — деньги переходят из BTC в альты через ETH. Если падает — сидим в BTC и ждём.",
    action: "ACCUMULATE x1.5 — усиливаем на откатах ETH/BTC.",
  },
  {
    id: "sol",
    kind: "crypto",
    symbol: "SOL",
    name: "Solana",
    sector: "Высокоскоростной L1",
    priceUsd: 186,
    change24h: 2.4,
    change7d: 6.7,
    allocationPct: 10,
    thesis:
      "Solana — крупнейший конкурент Ethereum по пользователям и DEX-обороту. RWA на Solana (Kamino + xStocks) открывает новый нарратив: токенизированные акции как залог.",
    howToRead:
      "SOL живёт циклами «разогнался — откатился 30%». Лучший вход — после коррекции к 200-дневной SMA при сохранении TVL и активности разработчиков.",
    action: "HOLD · x1 — откаты до -20% превращаем в DCA.",
  },
  {
    id: "gold",
    kind: "metal",
    symbol: "GOLD",
    name: "Золото (XAU/USD)",
    sector: "Физический резерв · антихрупкость",
    priceUsd: 2780,
    change24h: 0.3,
    change7d: 1.2,
    allocationPct: 12,
    thesis:
      "Страхование от дефолта фиата и геополитических шоков. Центробанки BRICS второй год подряд берут рекордные объёмы. Физическое золото не коррелирует с криптой и работает подушкой в фазе liquidity crunch.",
    howToRead:
      "Золото — тихий актив. Если оно обновляет ATH — мир нервничает, и это обычно предшествует подъёму BTC через 2-4 месяца.",
    action: "HOLD · физика или аллокированный XAU.",
  },
  {
    id: "oil",
    kind: "commodity",
    symbol: "OIL",
    name: "Нефть Brent",
    sector: "Сырьё · инфляционный хедж",
    priceUsd: 82.4,
    change24h: -0.8,
    change7d: 1.5,
    allocationPct: 5,
    thesis:
      "Нефть даёт информационный сигнал по мировому спросу и инфляции. Высокая нефть = давление на ставки = риск для длинных активов. Низкая нефть при растущей активности = бычий сигнал для риск-активов.",
    howToRead:
      "Нефть читаем как макро-термометр, а не как трейд. Растёт на фоне войны — краткосрочно плохо для крипты. Падает при растущем PMI — хорошо.",
    action: "HOLD · небольшая позиция, ребаланс раз в квартал.",
  },
  {
    id: "sp500",
    kind: "index",
    symbol: "SPX",
    name: "S&P 500 · индекс США",
    sector: "Широкий рынок акций",
    priceUsd: 5820,
    change24h: 0.4,
    change7d: 1.9,
    allocationPct: 8,
    thesis:
      "Индекс — «дыхание» риск-рынка. Когда S&P растёт при падающих ставках — крипта идёт следом. Через индекс мы диверсифицируем от одного сектора и одной страны (крипта слишком корреллирована сама с собой).",
    howToRead:
      "S&P 500 + DXY = карта ликвидности. Индекс вверх, доллар вниз — мёд для риск-активов. Индекс вниз, доллар вверх — плохой сезон.",
    action: "HOLD · индексный ETF, DCA каждый месяц.",
  },
];
